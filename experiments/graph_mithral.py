...
for data in itertools.chain(*data_sources[1:]):
    ...
    results_std = []
    for ncodebooks in [
        2**i for i in range(1, int(np.log2(data.info["biases"].size)) + 1)
    ]:
        print(f"ncodebooks={ncodebooks}")
        [W_test, W_train, X_test, X_train, Y_test, Y_train] = attrgetter(
            "W_test", "W_train", "X_test", "X_train", "Y_test", "Y_train"
        )(data)
        ...
        Y = data.info["lbls_test"][
            :align32
        ]  # True labels, cifar100 Y_hat is only 70% correct
        min_trials = []
        for _ in range(NAVG):
            trials = []
            for _ in range(NREPS):
                ...
                task = mithral_wrapped.mithral_amm_task_float(
                    *X_test.shape, W_test.shape[1], ncodebooks, lutconsts
                )
                task.amm.out_mat = np.zeros(task.amm.out_mat.shape)
        # How often the same as mat mutl
        # cpp_est_per_ix_kept=np.mean(cpp_max_ix==max_ix) 
        # This measure the accuracy on the last layer
        cpp_est_per_ix_kept=np.mean(cpp_max_ix==Y)
        o= MetricsSoftmax(np_time, py_fit_time, py_est_time, py_est_r2, py_est_per_ix_kept, copy_to_cpp_time, cpp_est_time, cpp_est_r2, cpp_est_per_ix_kept)
                ...
                task.run_matmul(
                    True
                )  # Encodes test Q as LUT instead of using train_Q's luts
                Y_hat2 = (
                    task.amm.out_mat
                )  # Since we just care about relative order for predicting output
    ...
    print(f"ncodebooks={ncodebooks}")
    print(
        f"Avg over {NAVG} of Min of {NREPS} Numpy Matrix mult times: {min_np_times}, {[i.np_time for i in results_std]}"
    )
    print(
        f"Avg over {NAVG} of Min of {NREPS} Mithral times: {min_mithral_times}, {[i.cpp_est_time for i in results_std]}"
    )
    print(
        f"Avg over {NAVG} of Min of {NREPS} cpp_est_per_ix_keeps: {cpp_est_per_ix_kepts}"
    )
    print(f"Numpy Matrix mult per_ix_keep, classifier accuracy {np.mean(Y==max_ix)}")
    print(
        f"cpp_est_per_ix_keeps/true accuracy of classifer : {[i/classifier_acc for i in cpp_est_per_ix_kepts]}"
    )
    print(
        f"cpp_est_per_ix_keeps - true accuracy of classifer : {[i - classifier_acc for i in cpp_est_per_ix_kepts]}"
    )
...
def compute_metrics_no_train(N, D, M, ncodebooks, X, Q):
    ...
    Y_hat = (
        task.amm.out_mat * ncodebooks / task.amm.out_scale
    ) + task.amm.out_offset_sum
...
def make_heatmap(x, y, vectorized_fn, Z1=None):
    ...
    times = extract_attr_from_metrics(Z1, "np_time") / extract_attr_from_metrics(
        Z1, "cpp_est_time"
    )
...
print(cpp_est_r2, cpp_est_per_ix_kept)
            ...
            np.array([i % 16] * (D // 2) + [(i % 16) for j in range(D // 2)])
            for i in range(N)
        ]
    )
    Q = np.stack(
        [np.array([i % 16] * (M // 2) + [16 + i % 16] * (M // 2)) for i in range(D)]
    )
...
print(
    "R2 of just the correct class for each row is worse than the total R^2",
    r2_score(Y[np.arange(len(Y)), max_ix], Y_hat2[np.arange(len(Y)), max_ix]),
    cpp_est_r2,
)
# %% See how good it is if it knows

task = mithral_wrapped.mithral_amm_task_float(
    *X_train.shape, W_train.shape[1], ncodebooks, lutconsts
)
task.amm.out_mat = np.zeros(task.amm.out_mat.shape)
t = time.perf_counter()
task.X = X_train
...
task.run_matmul(True)
Y_hat2 = (
    task.amm.out_mat[: Y.shape[0]] * ncodebooks / task.amm.out_scale
) + task.amm.out_offset_sum
cpp_est_r2 = r2_score(Y, Y_hat2)
cpp_max_ix = np.apply_along_axis(np.argmax, 1, Y_hat2)
cpp_est_per_ix_kept = np.sum(cpp_max_ix == max_ix) / cpp_max_ix.size
